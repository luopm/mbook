...
!function () {
    ...
    $.widget('ui.versionline', {
        ...
        _sort: function (){
            ...
            $.each(versionNodes, function(i, n) {
                if (n.effectTime) {
                    values.push(util.parse(n.effectTime, that.parseFormat).getTime());
                }
            });
            //排序
            ...
            $.each(versionNodes, function(i, n) {
                if (n.effectTime && (n.status == "retired" || n.status == "released" || !n.status)) {
                    $.each(values, function(j, m) {
                        if (util.parse(n.effectTime, that.parseFormat).getTime() == values[j]) {
                            sortChildren[j] = n;
                        }
                    });
                }
        ...
        _refreshValue: function () {
            ...
            if (this.options.versionNodes && this.options.versionNodes.length) {
                ...
                this.handles.each(function(i) {
                    ...
                    if (that.ranges[i]) {
                        if (that.orientation === "horizontal") {
                            if ((util.parse(showDate, that.parseFormat) < now) && (showDate != "--")) {
